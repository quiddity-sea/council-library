...
- [ ] A spend-task can be created with a projected cost, recurrence pattern, and budget link
- [ ] Marking a spend-task complete prompts for actual cost (projected pre-filled) and optionally creates a transaction with both values
- [ ] Fixed-number recurrence: after N completions, the task stops and no further instances are created
- [ ] Continuous recurrence: completing creates the next instance automatically; pausing halts recurrence; resuming restarts it; stopping ends it permanently
- [ ] Google Calendar: a recurring event is created on the designated Plutus calendar when a spend-task with recurrence is created
- [ ] Google Calendar: completing/pausing/resuming/stopping a task updates the corresponding Google Calendar event
- [ ] Google Calendar: OAuth token refresh is transparent — the user re-authorises only on expiry or revocation
- [ ] The dashboard shows upcoming spend-tasks with projected costs and recurrence labels
- [ ] Spend-tasks and regular tasks coexist in the same panel
- [ ] All existing tests (PHPUnit, PHPStan, Vitest, Playwright) pass after the changes
